...
struct sl_worker {
    ...
    uint32_t state;
    core_t *core;   // todo: replace with exec engine interface
...
int sl_worker_add_engine(sl_worker_t *w, core_t *c, uint32_t *id_out) {
    int err = sl_worker_add_event_endpoint(w, &c->event_ep, id_out);
    if (err) return err;

    if (w->core) sl_core_release(w->core);
    w->core = c;
    sl_core_retain(c);
    return 0;
}
