...
#define SLAC_IN_OP_RSUB         0x003   // reverse subtract
...
#define SLAC_IN_OP_BREAK        0x003   // breakpoint
...
struct sl_slac_inst {
    union {
        u4 raw;
        struct {
            u4 pred   : 4;  // 0  predicate - todo: remove
            u4 type   : 4;  // 4  type of instruction
            u4 arg    : 5;  // 8  arguments used
            u4 op     : 10; // 13 operation
            u4 len    : 3;  // 23 length of operands
            u4 sx8    : 1;  // 26 sign extend to u8
            u4 sh     : 1;  // 27 short instruction encoding (16 bit)
            u4 _unused: 4;  // 28
        };
    };
